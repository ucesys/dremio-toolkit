...
def rebuild_metadata(dremio_environment_url, user, password, root_point, concurrency, report_filename, log_level, log_filename):
    ...
    pds_list = get_pds_list(env_api, root_point)
    if pds_list is None:
...
def get_pds_list(env_api, root_point) -> list:
    sql = 'SELECT TABLE_SCHEMA, TABLE_NAME FROM INFORMATION_SCHEMA."TABLES" WHERE TABLE_TYPE = \'TABLE\''
    if root_point:
        sql += ' AND POSITION(\'' + root_point.lower() + '\' IN LOWER(TABLE_SCHEMA)) = 1'
